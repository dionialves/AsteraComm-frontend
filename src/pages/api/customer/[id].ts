...
export const GET: APIRoute = async ({ cookies, params }) => {
  ...
  try {
    const response = await fetch(`${config.api.baseUrl}/customers/${params.id}`, {
      headers: {
...
export const PATCH: APIRoute = async ({ cookies, params, url }) => {
  const token = cookies.get("token")?.value;
  ...
  try {
    const response = await fetch(`${config.api.baseUrl}/customers/${params.id}/${action}`, {
      method: "PATCH",
      headers: {
        Authorization: `Bearer ${token}`,
        Accept: "application/json",
